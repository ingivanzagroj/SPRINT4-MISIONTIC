    ...
    try:
        ...
        if request.method == "POST":    
            ...
            if not utils.isUsernameValid(username):
                error = "El username debe ser alfanumerico con '.','_','-'"
                flash(error)
                return render_template('auth/register.html')
            ...
            if db.execute('SELECT id FROM user WHERE email = ?', (email,)).fetchone() is not None:
                error =  'Email {} ya está registrado.'.format(email)
                flash(error)
                return render_template('auth/register.html')
            
            if (not utils.isPasswordValid(password)):
                error = 'La contraseña debe poseer una minúscula, una mayuscula, un numero de minimo 8 caracteres'
                flash(error)
                return render_template('auth/register.html')
            ...
            content = 'Hola, para activar tu cuenta por favor haz click en este link ' + flask.url_for('auth.activate', _external=True) + '?auth=' + number
            
            send_email(credentials, receiver=email, subject='Activate your account', message=content)
            
            flash('Por favor verifica tu Email para activar tu cuenta')
...
def confirm():
    try:
        ...
        if request.method == "POST": 
            ...
            if not password:
                flash('Password no puede estar vacío.')
            ...
            if  password1 != password:
                flash('Los valores no son los mismos')
                return render_template('auth/change.html', number=authid)
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            if ((not email) or (not utils.isEmailValid(email))):
                error = 'Dirección Email invalida'
                flash(error)
                return render_template('auth/forgot.html')
            ...
            if user is not None:
                ...
                db.execute(
                    "update fogotlink set state = ? where userid = ?",
                ...
                content = 'Hola, para cambiar tu contraseña por favor haz click en este link ' + flask.url_for('auth.change', _external=True) + '?auth=' + number
...
def login():
    try:
        ...
        if request.method == "POST":
            ...
            if user is None:
                error = 'Username o contraseña incorrectas'
